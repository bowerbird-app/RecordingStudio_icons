...
module RecordingStudioIcons
  module Generators
    class InstallGenerator < Rails::Generators::Base
      ...
      desc "Installs RecordingStudioIcons engine into your application"
      ...
      def add_tailwind_source
        ...
        unless File.exist?(tailwind_css_path)
          ...
          say '  @source "../../vendor/bundle/**/recording_studio_icons/app/views/**/*.erb";', :yellow
          return
        end

        tailwind_content = File.read(tailwind_css_path)
        source_line = '@source "../../vendor/bundle/**/recording_studio_icons/app/views/**/*.erb";'

        if tailwind_content.include?(source_line)
          say "Tailwind already configured to include RecordingStudioIcons views.", :green
          return
        end
        ...
        if tailwind_content.include?('@import "tailwindcss"')
          inject_into_file tailwind_css_path, after: "@import \"tailwindcss\";\n" do
            "\n/* Include RecordingStudioIcons engine views for Tailwind CSS */\n#{source_line}\n"
          end
          say "Added RecordingStudioIcons views to Tailwind CSS configuration.", :green
